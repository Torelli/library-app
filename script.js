...
class Library {
    ...
    if (this.#books != []) {
      for (let book of this.#books) {
        ...
        btnRead.setAttribute(
          "id",
          `btn-isRead-${this.#books.findIndex(
            (obj) => obj.title === book.title && obj.author === book.author
          )}`
        );
        ...
        btnDelete.setAttribute(
          "id",
          `btn-delete-${this.#books.findIndex(
            (obj) => obj.title === book.title && obj.author === book.author
          )}`
        );
  ...
  addBook = (title, author, description, isRead) => {
    if (
      this.#books.some((book) => title === book.title && author === book.author)
    ) {
      ...
      this.#books.push(new Book(title, author, description, isRead));
      this.displayBooks();
      toggleModal();
    }
  };
